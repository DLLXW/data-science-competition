...
import datetime
from dataset_triplet import DogDataset
...
class Config(object):
    ...
    warmup_epoch=3
    max_epoch = 60# T0=2:6/14/30/62 ; T0=3:9/21/45/93
    T0=2
    ...
    load_from = None
    #
    checkpoints_dir = 'ckpt_0521_triplet/'
...
def train_one_epoch(fold,epoch, model, loss_fn, optimizer, train_loader, device, scheduler=None, schd_batch_update=False):
    ...
    #valid_one_epoch(0, 0, model, loss_fn, val_loader, device)
    for step, data in enumerate(train_loader):
        imgA=data['image_A'].to(device).float()
        imgB=data['image_B'].to(device).float()
        imgC=data['image_C'].to(device).float()
        #image_labels_neg_1 = data['label_neg_1'].to(device).long()
        #image_labels_neg_2 = data['label_neg_2'].to(device).long()
        image_labels_pos = data['label_pos'].to(device).long()
        #print(image_labels_pos,1-image_labels_pos)
        #
        with autocast():
            dis_pos,dis_neg,frt_A,frt_B,frt_C  = model(imgA,imgB,imgC)
            target = torch.FloatTensor(dis_neg.size()).fill_(1)
            target = target.cuda()
            target = Variable(target)
            #当target=1的时候，表示我们预期frt_neg的排名要比frt_pos高，也就是frt_neg-frt_pos>0
            loss_triplet = criterion_triplet(dis_neg,dis_pos,target)
            loss_embedd = frt_A.norm(2) + frt_B.norm(2) + frt_C.norm(2)
            ...
            if step % opt.print_interval == 0 or dis_pos.size()[0] < opt.batch_size:
                spend_time = time.time() - begin_time
                logger.info(
                    'Fold:{} Epoch:[{}/{}]({}/{}) loss:{:.3f} loss triplet:{:.3f} loss_embedd:{:.3f} lr:{:.7f} epoch_Time:{}min:'.format(
                        fold,epoch,opt.max_epoch, step, epoch_iters,loss.item(),
                        loss_triplet.item(),loss_embedd.item(), optimizer.param_groups[-1]['lr'],
...
def valid_one_epoch(fold, epoch, model, loss_fn, val_loader, device, scheduler=None, schd_loss_update=False):
    ...
    for step, data in enumerate(val_loader):
        ...
        #image_preds=F.softmax(image_preds,dim=1).detach().cpu().numpy()[:,1]
...
if __name__ == '__main__':
    ...
    shutil.copy('./dataset.py',save_dir+'/dataset.py')
    ...
    if opt.use_kfold:
        for fold, (trn_idx, val_idx) in enumerate(folds):
            ...
            criterion_triplet = torch.nn.MarginRankingLoss(margin = 0.2)
