...
class DogDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, df,df_sim, root_dir,transform=None,mode='train'):
        ...
        self.mode = mode
        self.df_sim=df_sim
        self.df_byb=pd.read_csv('/home/yangdeyu/competition/qyl/code_classify/byb.csv')
        self.root_byb='/home/yangdeyu/competition/qyl/data/pet_biometric_challenge_2022/test/test/'
        self.df_byb_A=pd.read_csv('/home/yangdeyu/competition/qyl/code_classify/byb_A.csv')
        self.root_byb_A='/home/yangdeyu/competition/qyl/data/pet_biometric_challenge_2022/validation/images/'
    ...
    def __getitem__(self, idx):
        ...
        if random.random() < 0.5:
            if random.random() < 0.5:
                row_byb=self.df_byb.iloc[idx//24]#imageA,imageB,prediction
                name_A=row_byb['imageA']
                name_B=row_byb['imageB']
                label=int(row_byb['prediction']>1000)
                flag=1
            else:
                row_byb=self.df_byb_A.iloc[idx//24]#imageA,imageB,prediction
                name_A=row_byb['imageA']
                name_B=row_byb['imageB']
                label=int(row_byb['prediction']>1000)
                flag=2
        ...
        if flag==0:
            ...
            image_A = cv2.imread(os.path.join(self.root_byb, name_A))
            image_A = cv2.cvtColor(image_A, cv2.COLOR_BGR2RGB)
            image_B = cv2.imread(os.path.join(self.root_byb, name_B))
            image_B = cv2.cvtColor(image_B, cv2.COLOR_BGR2RGB)
        else:
            image_A = cv2.imread(os.path.join(self.root_byb_A, name_A))
            image_A = cv2.cvtColor(image_A, cv2.COLOR_BGR2RGB)
            image_B = cv2.imread(os.path.join(self.root_byb_A, name_B))
            image_B = cv2.cvtColor(image_B, cv2.COLOR_BGR2RGB)
