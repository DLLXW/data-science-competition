...
if __name__=='__main__':
    ...
    #
    scheduler = torch.optim.lr_scheduler.LambdaLR(optimizer, lr_lambda=warm_up(warm_up_epochs=3))
    ...
    for epoch in range(1,max_num_epochs):
        ...
        for step, (token_ids, label) in enumerate(train_dataloader):
            ...
            loss.backward()  # 反向传播
            optimizer.step()  # 梯度更新
            scheduler.step(epoch + step / total_iters)
            loss_sum += loss.cpu().data.numpy()
            accu += (out.argmax(1) == label).sum().cpu().data.numpy()
            if step%print_interval==0 and step!=0:
                spend_time=time.time()-begin_time
                print("epoch{},[{}/{}],train_loss:{:.4},lr:{:.4},ETA:{}min".format(epoch,step,total_iters,loss,optimizer.param_groups[-1]['lr'],spend_time/ step * total_iters // 60 - spend_time // 60))
        #
        scheduler.step()
        test_loss_sum = 0.0
        test_acc = 0
