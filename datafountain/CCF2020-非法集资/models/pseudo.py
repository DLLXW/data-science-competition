...
def lgb_tta(tta_fold,train_data,kind,test_data):
    score_tta = None
    score_list_F = []
    score_list_f1 = []
    score_list_p = []
    ...
    for _ in tqdm(range(tta_fold)):
        clf = lgb.LGBMClassifier(
            ...
            n_estimators=150, n_jobs=-1,silent=True)
...
if __name__=="__main__":
    #获取特征
    use_intersection_cv=True
    use_intersection = False
    train_df=pd.read_csv("../usr_data/train_data.csv")
    test_df=pd.read_csv("../usr_data/test_data.csv")
    # #特征筛选
    select_features ,cat_features=select_frt_names(train_df,thres=0.2)
    train_data = train_df[select_features]
    kind=train_df['label']
    test_data=test_df[select_features]
    pseudo_name = []
    pseudo_pos=0
    pseudo_label = {'id': [], 'label': []}
    #pseudo_df = pd.read_csv('D:/非法集资/pycharm_project/submission/lgb_pseudo_847_934.csv')#0.9,0.002 0.8448
    pseudo_df = pd.read_csv('D:/非法集资/pycharm_project/submission/submit_8485.csv')
    for index, name, score in pseudo_df.itertuples():
        if score > 0.7 or score < 0.01:
    ...
    #
    print(len(pseudo_name),pseudo_pos,len(pseudo_name)/pseudo_pos)
    ...
    train_data = train_df[select_features]
    kind = train_df['label']
    print("pseudo:",train_data.shape, test_data.shape)
    #
    score_tta,f_1=lgb_tta(30,train_data,kind,test_data)
    cnt_re = cnt_result(score_tta)
    print("合法的数量%d;违法的数量%d,合法/违法%f" % (cnt_re[0], cnt_re[1], cnt_re[0] / cnt_re[1]))
    test_data_sub = test_df.copy()
    score_tta_int = (score_tta > 0.5).astype(int)
    test_data_sub['score'] = score_tta
    test_data_sub[['id', 'score']].to_csv('../submission/lgb_122_' + str(int(1000*f_1))+'_'+ str(cnt_re[1]) + '.csv', index=None)
