...
if __name__ == "__main__":
    ...
    #
    model_name='lstm'#cnn lstm
    model_save_dir ='ckpt_0420_'+model_name+'_'+bert_type+'_fine_attr/'
    only_attr=True
    only_tuwen=False
    ...
    shutil.copy('./train_cnn_lstm_attr.py',model_save_dir+'/train_cnn_lstm_attr.py')
    if only_attr:
        class_num=12
    else:
        class_num=13
    ...
    for fold, (trn_idx, val_idx) in enumerate(folds):
        ...
        if model_name=='cnn':
            model=CNN_Text(bert_name,class_num=class_num)
        else:
            model=LSTM_Text(bert_name,class_num=class_num)
        model.to(device)
        labels_fine_train=[]
        labels_fine_val=[]
        for inx in trn_idx:
