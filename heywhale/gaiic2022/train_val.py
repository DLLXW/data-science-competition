...
def train_model(model,criterion, optimizer, lr_scheduler=None):
    ...
    for epoch in range(1,max_epoch+1):
        ...
        val_auc,tuwen_acc,attr_acc= val_model(model, criterion)
        val_acc=0.5*tuwen_acc+0.5*attr_acc
        print('val auc: {:.4f} val acc: {:.4f} tuwen_acc: {:.4f} attr_acc: {:.4f}'.format(val_auc,val_acc,tuwen_acc,attr_acc))
        ...
        if val_acc > best_acc:
            ...
            print("save best epoch: {} best auc: {} best acc: {}".format(best_epoch,val_auc,best_acc))
        #save based on epoch interval
        #if epoch % 5  == 0 and epoch>30:
            #torch.save(model.state_dict(), model_out_path)
    #
    print('Fold{} best_acc: {:.3f} Best epoch:{}'.format(fold+1,best_acc,best_epoch))
