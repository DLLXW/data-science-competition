...
class BartDataset(torch.utils.data.Dataset):
    def __init__(self, name, indexed_dataset, data_prefix,
        ...
        # Denoising ratios
        self.permute_sentence_ratio = 0.0
        self.mask_ratio = masked_lm_prob  # 0.3
    ...
    def build_training_sample(self, sample, max_seq_length, np_rng):
        ...
        tokens = []
        for sent in sample:
            if len(tokens) + len(sent) + 2 > max_seq_length:
                break
            for t in sent:
                tokens.append(t)
                full_stops.append(0)
        ...
        full_stops = [0] + full_stops + [0]
        tokens = torch.LongTensor(tokens)
        full_stops = torch.LongTensor(full_stops)
        ...
        if self.mask_ratio > 0.0:
            ...
            source = self.add_whole_word_mask(source, self.mask_ratio, replace_length)
    ...
    def word_starts(self, source):
        ...
        words = [raw_tokens[0]] + self.zh_tokenizer(''.join(raw_tokens[1:-1])) + [raw_tokens[-1]]
