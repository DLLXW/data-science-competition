...
from megatron import get_num_microbatches, get_args
...
class AnnealingLR(object):
    ...
    def __init__(self, optimizer, min_lr,
        ...
        for pg in self.optimizer.param_groups:
            self.max_lrs.append(pg['lr'])
        self.max_lr = max(self.max_lrs)
        self.min_lr = min_lr
    ...
    def get_lr(self, i=0):
        ...
        if self.warmup_steps > 0 and self.num_steps <= self.warmup_steps:
            return self.max_lrs[i] * float(self.num_steps) / \
                float(self.warmup_steps)

        # If the learning rate is constant, just return the initial value.
        if self.decay_style == 'constant':
            return self.max_lrs[i]
        ...
        delta_lr = self.max_lrs[i] - self.min_lr
    ...
    def load_state_dict(self, sd):
        ...
        max_lrs_ = sd['max_lrs']
        self.max_lrs = max_lrs_
        self.max_lr = max(max_lrs_)
