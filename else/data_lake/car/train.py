...
class Config(object):
    seed=42 # swin_large_patch4_window12_384
    backbone = 'swin_large_patch4_window7_224'# swin_large_patch4_window7_224 'convnext_large_in22ft1k' tf_efficientnet_b5_ns,tf_efficientnet_l2_ns,vit_base_patch16_384
    num_classes = 21 #
    ...
    warmup_epoch= 3
    max_epoch = 15 # T0=2:6/14/30/62 ; T0=3:9/21/45/93
    T0=3
...
def prepare_dataloader(df, trn_idx=None, val_idx=None, data_root=None,use_kfold=False):
    ...
    if use_kfold==False:
        ...
        valid_=df
        train_ds = RubbishDataset(train_, data_root, transforms=get_train_transforms(opt.height_size,opt.width_size), output_label=True,mode='train')
        valid_ds = RubbishDataset(valid_, data_root, transforms=get_valid_transforms(opt.height_size,opt.width_size), output_label=True,mode='val')
...
def calculat_f1(output, target):
    upperLength_output=output[:,:7]
    clothesStyles_output=output[:,7:17]
    hairStyles_output=output[:,17:]
    upperLength_target=target[:,:7]
    clothesStyles_target=target[:,7:17]
    hairStyles_target=target[:,17:]
    #
    ...
    #
    f1_list = [upperLength_f1,clothesStyles_f1,hairStyles_f1]
    return f1_list
@torch.no_grad()
...
if __name__ == '__main__':
    ...
    logger.info('Using cutmix: {}'.format(opt.cutmix))
    f1_lst=[]
    ...
    if opt.use_kfold:
        for fold, (trn_idx, val_idx) in enumerate(folds):
            ...
            if opt.lr_scheduler=='StepLR':
                ...
                scheduler = torch.optim.lr_scheduler.CosineAnnealingWarmRestarts(optimizer, T_0=opt.max_epoch-opt.warmup_epoch+1, T_mult=1, eta_min=1e-7, last_epoch=-1)
