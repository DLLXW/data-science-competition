...
def slice_im(image_path, sliceHeight=1024, sliceWidth=1024,overlap=0.2):
    #
    result_pre=[]
...
if __name__ == '__main__':
    ...
    parser.add_argument('--slice_size', type=int, default=2048, help='inference size (pixels)')
    ...
    for per_img_name in tqdm(os.listdir(opt.source)):
        ...
        #
        image_result_pre,win_h, win_w = slice_im(image_path, sliceHeight=opt.slice_size, sliceWidth=opt.slice_size)
        # print(image_result_pre)
        boxes_list =[]
        scores_list=[]
        labels_list=[]
        for per_pre in image_result_pre:
            boxes_list.append([per_pre[0]/win_w,per_pre[1]/win_h,per_pre[2]/win_w,per_pre[3]/win_h])
            scores_list.append(per_pre[5])
            labels_list.append(per_pre[4])
        ...
        if image_result_pre != [[]]:
            for i in range(len(boxes)):
                ...
                submit_result.append(
                    {'name': per_img_name, 'category': int(labels[i]), 'bbox': box, 'score': float(str(scores[i])[:5])})
    #
    print(submit_result)
    if not os.path.exists('results/'):os.makedirs('results/')
    with open('results/resut_wbf_2048.json', 'w') as fp:
